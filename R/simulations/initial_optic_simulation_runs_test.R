...
devtools::load_all("/Users/plima/dev/optic/optic-core")
library(dplyr)
library(here)
library(augsynth)
library(future)
...
write.csv(results, 
          file = file.path(here::here(), "data/simulations/opptic_raw_results.csv"),
          row.names = FALSE)

# Print warnings to warnings.txt file:call.
warnings_file <- file(file.path(here::here(), "data/simulations/opptic_warnings.txt"), open = "wt")
writeLines(capture.output(warnings()), warnings_file)
close(warnings_file)  